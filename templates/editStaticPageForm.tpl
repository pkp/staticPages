...
<script>
	$(function() {ldelim}
		...
		$('#staticPageForm').pkpHandler(
			...
			{ldelim}
				previewUrl: {url|json_encode router=PKPApplication::ROUTE_PAGE page="pages" op="preview"}
			{rdelim}
		);
	{rdelim});
</script>

{capture assign=actionUrl}{url router=PKPApplication::ROUTE_COMPONENT component="plugins.generic.staticPages.controllers.grid.StaticPageGridHandler" op="updateStaticPage" existingPageName=$blockName escape=false}{/capture}
<form class="pkp_form" id="staticPageForm" method="post" action="{$actionUrl}">
	...
	{fbvFormArea id="staticPagesFormArea" class="border"}
		...
		{/fbvFormSection}
		{fbvFormSection}
			{capture assign="exampleUrl"}{url|replace:"REPLACEME":"%PATH%" router=PKPApplication::ROUTE_PAGE context=$currentContext->getPath() page="REPLACEME"}{/capture}
